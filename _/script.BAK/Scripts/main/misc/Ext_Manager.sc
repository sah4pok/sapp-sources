...
Ext_Manager:
	...
	//---MAIN LOOP---
Ext_Manager_Main_Loop:
	
	IF IS_PLAYER_PLAYING PLAYER1
		GOSUB Ext_Manager_State_Machine
	ENDIF
		
	WAIT 0

GOTO Ext_Manager_Main_Loop 
	//---END OF MAIN LOOP---

/********************************************
			STATE MACHINE
********************************************/
Ext_Manager_State_Machine:
	SWITCH iState	   	
   		CASE 0 //---STATE 0 - Waiting to enter an entry exit
			GOSUB Ext_Manager_ENTRY
		BREAK
   		CASE 1 //---STATE 1 - Waiting to leave an entry exit
			GOSUB Ext_Manager_EXIT
		BREAK
	ENDSWITCH
RETURN
/********************************************
	...
	IF NOT iAreaCode = 0 
		...
		IF NOT IS_STRING_EMPTY $txtEntryExit
			...
			OR $txtEntryExit = &AMMUN5
				GET_NUMBER_OF_INSTANCES_OF_STREAMED_SCRIPT ammu3.sc iStreamedScriptInstances
				IF iStreamedScriptInstances = 0
					STREAM_SCRIPT ammu3.sc
					LOAD_ALL_MODELS_NOW
					IF HAS_STREAMED_SCRIPT_LOADED ammu3.sc
						START_NEW_STREAMED_SCRIPT ammu3.sc
						iCreateDecisionMakers = 1
						++iState // move on to the wait state							
					ENDIF
				ENDIF
...
Ext_Manager_EXIT:  
	...
	IF NOT iAreaCode = iStoredAreaCode
		...
		GOSUB Ext_Manager_MarkStuffNoLongerNeeded
...
********************************************/
Ext_Manager_MarkStuffNoLongerNeeded:
	//--- Animations
	REMOVE_ANIMATION INT_SHOP
	//--- Shop scripts	
	MARK_STREAMED_SCRIPT_AS_NO_LONGER_NEEDED ammu3.sc 
	//--- Decision Makers					
